...
export default function AnalysisClient({ data }: { data: AnalysisData }) {
  ...
  const { domains, yearlyAnalysis, statistics } = data;
        ...
        <div className="flex flex-col gap-4">
          {domains.map((domain) => {
            ...
            return (
              <div
                key={domain.id}
                ref={(el) => { domainRefs.current[domain.id] = el; }}
                className={`scroll-mt-4 rounded-xl border bg-white transition ${isOpen ? `${colors.border} shadow-md` : "border-stone-200"}`}
              >
                <button
